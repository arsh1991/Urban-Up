...
function onGABLoaded(){
  ...
  //adding eventListener to autoRelease to filter true or false rows
  defaultReleaseFilter.addEventListener("click", filterOnDefaultRelease);
...
var filteredDefaultTableRows = [];
var defaultFilteredOn = "not";
////////////////////////////////////////////////////////////////////////////////
...
function filterTableOnAddresses(){
  ...
  tableBodyChildrenArray.forEach(function(child){
    ...
    var workerAddressString = child.querySelector(".workerAddress").innerText.trim();
    ...
    && !workerAddressString.startsWith(String(addressFilter.value.trim()))){
...
function filterOnDefaultRelease(){
  fourthCol = document.querySelectorAll(".autoRelease");
  restoreStateFilteredDefaultRows();
  switch(defaultFilteredOn){
    case "not":
      defaultFilteredOn = "false";
      $('#autoReleaseFilter').text(": true");
      break;
      case "false":
        defaultFilteredOn = "true";
        $('#autoReleaseFilter').text(": false");
        break;
      case "true":
        defaultFilteredOn = "not";
        $('#autoReleaseFilter').text("");
  ...
  fourthCol.forEach(function(element){
    if(element.innerText.trim() === defaultFilteredOn){
      filteredDefaultTableRows.push(element.parentNode.parentNode.parentNode);
      element.parentNode.parentNode.parentNode.parentNode.removeChild(element.parentNode.parentNode.parentNode);
    }
  });
}

function restoreStateFilteredDefaultRows(){
  var filteredDefaultTableRowsLength = filteredDefaultTableRows.length;
  for(var counter = 0; counter < filteredDefaultTableRowsLength; counter++){
    var childToBeAdded = filteredDefaultTableRows.pop();
...
function restoreStateFilteredRows(){
  var filteredTableRowsLength = filteredTableRows.length;
